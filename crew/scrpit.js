// DANE CZŁONKÓW EKIPY (Przykładowe miejsce na Twoje materiały promocyjne)
const CREW_MEMBERS = [
    {
        id: 1,
        name: "Śrubecki",
        role: "Founder / DJ / Turntablist",
        bio: "Twórca projektu SPOKULTURA, pomysłodawca Loopera i pasjonat sztuki gramofonowej.",
        avatar: "../assets/images/crew/srubecki.jpg", // ścieżka do zdjęcia
        socials: {
    ...
    {
        id: 2,
        name: "Nazwa Członka 2",
        role: "Rola / Definicja",
        bio: "Krótki opis osiągnięć, roli w projekcie lub zajawki jaką wkurwia społeczność.",
        avatar: "../assets/images/crew/placeholder.jpg",
        socials: {
            instagram: "https://instagram.com",
...
// GENEROWANIE IKON DLA SOCIAL MEDIA
...
function renderCrew() {
    ...
    CREW_MEMBERS.forEach(member => {
        ...
        // Tworzenie linków social media
        let socialsHtml = "";
        if (member.socials) {
            Object.entries(member.socials).forEach(([platform, url]) => {
                if (url) {
                    socialsHtml += `
                        <a href="${url}" target="_blank" rel="noopener noreferrer" class="crew-social-btn" aria-label="${platform}">
        ...
        card.innerHTML = `
            <div class="crew-avatar-wrapper">
                <img src="${member.avatar}" alt="${member.name}" class="crew-avatar" onerror="this.src='https://via.placeholder.com/160/111111/ff6a00?text=${encodeURIComponent(member.name)}'">
            </div>
